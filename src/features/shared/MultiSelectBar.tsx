...
export function MultiSelectBar({ count, busy, projects, onComplete, onReopen, onDelete, onAddToToday, onRemoveFromToday, onMoveToGroup, onExit }: Props) {
  ...
  return <div className="multi-select-bar" role="group" aria-label="批量操作">
    ...
    {onAddToToday && <button onClick={onAddToToday} disabled={busy}>加入今日</button>}
    {onRemoveFromToday && <button onClick={onRemoveFromToday} disabled={busy}>移出今日</button>}
